...
describes what users get. Pick a format with `-o`, or let travelokavn choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
travelokavn <command> -o table   # aligned columns for reading
travelokavn <command> -o jsonl   # one JSON object per line, for piping
travelokavn <command> -o json    # a single JSON array
travelokavn <command> -o csv     # spreadsheet friendly
travelokavn <command> -o tsv     # tab-separated
travelokavn <command> -o url     # just the URL column
travelokavn <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
travelokavn <command> --fields id,title,url
```
...
```bash
travelokavn <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
travelokavn <command>            # a table, because this is a terminal
travelokavn <command> | wc -l    # JSONL, because this is a pipe
```
